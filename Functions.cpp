...
int initSystem(string title){
    ...
    screen = SDL_CreateWindow(title.c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, screen_width, screen_height, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    renderFlags = SDL_RENDERER_ACCELERATED;
    font = TTF_OpenFont("Sprites/joystix_monospace.otf", 60); //Loading text font, the first parameter indicates the font and the second one is the size.
    smaller_font = TTF_OpenFont("Sprites/joystix_monospace.otf", 30);
...
void drawFrame(string id, int columnFrame, int rowFrame, int width, int height, int x, int y, bool show_hitbox, const SDL_RendererFlip flip){
    SDL_Rect r;
    r.x = x;
    r.y = y;
    SDL_Rect image_r;
    ...
    SDL_RenderCopyEx(render, textures[id], &image_r, &r, 0, 0, flip);
    if(show_hitbox == true){
        draw_rect(r);
    }
}
